    ...
    def get_valid_moves(self, curr_pos: str) -> list:
        ...
        if piece[-1] == "K":
            ...
            if not self.is_in_check(color, piece_x, piece_y): # king cannot castle if in check!
                ...
                # if not rook_in_check and pieces_not_moved and all(no_pieces_blocking_castling)
                if not self.is_in_check(color, 7, num) and castling[0] and all(self.board[num][i] == "  " for i in range(5, 7)):
                    valid_moves.append(f"6{num} O-O")
                # Queen's side castling (O-O-O)
                if not self.is_in_check(color, 0, num) and castling[1] and all(self.board[num][i] == "  " for i in range(1, 4)):
